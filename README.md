...
An end-to-end ASR evaluation and fine-tuning pipeline for the Azerbaijani language, built with OpenAI Whisper and the Mozilla Common Voice 17.0 dataset.
...
Baseline evaluation of whisper-small on Azerbaijani test speech

B

Fine-tuning whisper-tiny on a small Azerbaijani training subset

C

Analytical report (in Azerbaijani) covering results, challenges, and future work
...
MP3 (resampled to 16 kHz at runtime)

Approximate train hours

~14h (varies by version)
...
Best accuracy/speed trade-off for zero-shot Azerbaijani; ~244M params

Fine-tuning

openai/whisper-tiny

Fits comfortably on free Colab GPU; fast iteration; ~39M params
...
where $S$ = substitutions, $D$ = deletions, $I$ = insertions, $N$ = total words in reference.WER measures how many words the model got wrong at the word level. Lower is better.
...
See report.md for the full analytical report in Azerbaijani.
...
This project is for educational/internship purposes only.Mozilla Common Voice dataset is licensed under CC-0.Whisper model weights are licensed under Apache 2.0.
